...
fNameFreq = 'C:\work\examples\wg_perm_sym\results\optMaterials\eps20_f.txt';
VectorWriter(f,fNameFreq);
fNameS = 'C:\work\examples\wg_perm_sym\results\optMaterials\eps20_sPar.txt';
...
% % plot of costs as a function of muReal
% costVal = [];
% for k=1:0.5:15
%   muVec = [k,-1];
%   [v, d] = costFunGrad(muVec);
%   costVal = [costVal, v];
% end
% plot(k, costVal);
...
% start optimization %
%%%%%%%%%%%%%%%%%%%%%%
muStart = [4,0]; % Make a starting guess at the solution
...
lowBndMu = [1, -10];
upBndMu = [25, 0];
% [muEst, fval] = fmincon(@costFun,muStart,[],[],[],[],lowBnd,upBnd,[],options)
[muEst, fval] = fmincon(@costFun,muStart,[],[],[],[],lowBndMu,...
  upBndMu,[],options)

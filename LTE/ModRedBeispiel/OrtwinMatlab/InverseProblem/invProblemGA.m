...
% filename = 'G:\invProblems\wg_perm_sym_1.2e+010_10_10_pN\s_11_f_8e+009_1.7e+010_101_m_5-5e-005i_5-5e-005i_1.txt';
...
% sRef = sRef+(randn(length(sRef),1)+j*randn(length(sRef),1))*1e-5;

% filenameOut = 'C:\work\examples\wg_perm_sym\results\optMaterials\eps2.6_sPar.txt';
% vectorWriterComplex( sRef, filenameOut );
% 
% filenameOut = 'C:\work\examples\wg_perm_sym\results\optMaterials\eps2.6_f.txt';
% vectorWriter( f, filenameOut );

pathName = 'G:\invProblems\wg_perm_sym_1.2e+010_10_10_pN\';
...
options = gaoptimset('PopInitRange',initRange);
options = gaoptimset(options,'StallTimeLimit',200);

[muEst, fval, reason, output] = ga(@costFun,2,options)

...
% end
% plot(fr*1e-9, 20*log10(abs(s11)), 'b-', 'LineWidth', linewidth);
% hold;
% grid;
% 
% % model with one TFE Port and Radiation BCs
% fNameWsolver = ...
%   [Path 'sParamRadiation.txt'];
% [fr sMatWaveSolv] = readSparamWaveSolverCell(fNameWsolver);
% s11 = zeros(length(fr),1);
% for k = 1:length(fr)
%   s11(k) = sMatWaveSolv{k}(1,1);
% end
% plot(fr*1e-9, 20*log10(abs(s11)), 'r-', 'LineWidth', linewidth);

% % model with one TFE Port, Radiation BCs and PECs
% fNameWsolver = ...
%   [Path 'sParamPEC.txt'];
% [fr sMatWaveSolv] = readSparamWaveSolverCell(fNameWsolver);
% s11 = zeros(length(fr),1);
...
%   s11(k) = sMatWaveSolv{k}(1,1);
% end
% plot(fr*1e-9, 20*log10(abs(s11)), 'g-', 'LineWidth', linewidth);
% hold;
% grid;
...
plot(freqs*1e-9, 20*log10(abs(sVal)), 'c', 'LineWidth', linewidth);
...
plot(freqs*1e-9, 20*log10(abs(sVal)), 'y', 'LineWidth', linewidth);

% modelName = ...
% 'C:\work\examples\SMA_Accurate\SMA_Accurate_1e+010_20_Lossy\';
...
% end
% plot(freqs*1e-9, 20*log10(abs(sValO5)), 'c-', 'LineWidth', linewidth);
...
legend('WaveSolver', 'WCAWE: order 30', 'WCAWE: order 50', 'Frequency model: order 10', ...
  'Frequency + material model: order 5', 'Location', 'SouthEast');
...
'C:\work\examples\SMA_Accurate\SMA_Accurate_1e+010_EPSILON_RELATIVE_dielec_01_02_(3.8,0)_EPSILON_RELATIVE_dielec_05_06_(3.8,0)_5\';
% fNameSpara = modelEvaluationGeneral(modelName);
fNameSpara = [modelName 'S_f_1e+009_2e+010_191_EPSILON_RELATIVE_dielec_01_02_(1,0)_(8,0)_1_EPSILON_RELATIVE_dielec_05_06_(1,0)_(8,0)_1.txt'];
[parameterNames, numParameterPnts, parameterVals, sMatrices] ...
    = loadSmatrix(fNameSpara);
% plot results
sValO5 = zeros(numParameterPnts(1),1);
...
legend('WaveSolver', 'ROM order 5', 'Location', 'SouthEast');


% modelName = ...
% 'C:\work\examples\SMA_Accurate\SMA_Accurate_1e+010_EPSILON_RELATIVE_dielec_01_02_(3.8,0)_EPSILON_RELATIVE_dielec_05_06_(3.8,0)_8\';
% buildRedModelIdent_ModScal(modelName, 5);

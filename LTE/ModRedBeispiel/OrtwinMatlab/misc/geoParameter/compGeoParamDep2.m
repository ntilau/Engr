...
sysMatIntp = cell(length(sIntPnts{1}), length(sIntPnts{2}));
k2MatIntp = cell(length(sIntPnts{1}), length(sIntPnts{2}));
for s1Cnt = 1:length(sIntPnts{1})
  for s2Cnt = 1:length(sIntPnts{2})
    fNameSysMat = strcat(modelNameOriginalBase, ...
      num2str(sIntPnts{1}(s1Cnt)), '_dz_', ...
      num2str(sIntPnts{2}(s2Cnt)), '\system matrix');
    sysMatIntp{s1Cnt, s2Cnt} = MatrixMarketReader(fNameSysMat);
    fNameK2Mat = strcat(modelNameOriginalBase, ...
      num2str(sIntPnts{1}(s1Cnt)), '_dz_', ...
      num2str(sIntPnts{2}(s2Cnt)), '\k^2 matrix');

function [Q U] = wcaweVS2(fact, model, order)
...
%       v = -model.sysMat{m + 1} * (Q(:, 1 : (n - m)) * (PUwInverse(U, 2, m, n) \ eNmM));
      v = -model.sysMat{m + 1} * (Q(:, 1 : (n - m)) * solveLinSysEq(PUwInverse(U, 2, m, n), eNmM));
    else
%       v = v - model.sysMat{m + 1} * (Q(:, 1 : (n - m)) * (PUwInverse(U, 2, m, n) \ eNmM));
      v = v - model.sysMat{m + 1} * (Q(:, 1 : (n - m)) * solveLinSysEq(PUwInverse(U, 2, m, n), eNmM));    
    end
  end
  ...
  for m = 1 : min(length(model.rhs) - 1, n - 1)
    if (m + 1) <= length(model.rhs)
      ...
      col = solveLinSysEq(pu2, eNmM);
      if nnz(model.rhs{m + 1})
        if ~nnz(v)
%           v = pu2(1, n - m) \ model.rhs{m + 1};
%           v = solveLinSysEq(pu2(1, n - m), model.rhs{m + 1});
          v = col(1) * model.rhs{m + 1};
        else
%           v = v + pu2(1, n - m) \ model.rhs{m + 1};
%           v = v + solveLinSysEq(pu2(1, n - m), model.rhs{m + 1});
...
function x = solveLinSysEq(mat, rhs)

...
cLow = cell(dimLow, dimLow);
for rowCnt = 1:dimLow
  for colCnt = 1:dimLow
    cLow{rowCnt, colCnt} = c{rowCnt, colCnt};
  end
end
...
% order (first x then y or vice versa) in wich backConvertTscheby 
% is called doesn't make a difference
dLow = backConvertTscheby2dCell(a1, b1, a2, b2, cLow);
display(dLow);

finalPolyLow = [];
maxOrder = dimLow + dimLow - 2;
numParams = 2;
for k = 0:maxOrder
  finalPolyLow = rec(numParams, k, finalPolyLow, 0, 1);
end
coeffPolyLow = zeros(size(finalPolyLow, 1), length(f{1, 1}));

for x1Cnt = 1:dimLow
  for x2Cnt = 1:dimLow
    rowNow = [(x1Cnt - 1) (x2Cnt - 1)]; 
    rowPos = findRowInMat(rowNow, finalPolyLow);
    coeffPolyLow(rowPos, :) = dLow{x1Cnt, x2Cnt};
  end
end

fTestLow = zeros(length(x1), length(x2));
for s1Cnt = 1:length(x1)
  for s2Cnt = 1:length(x2)
    sAct = [x1(s1Cnt) x2(s2Cnt)];
    fNow = 0;
    for rowCnt = 1:size(finalPolyLow, 1)
      ...
      fNow = fNow + coeffPolyLow(rowCnt, :) .* pow ;
    end
    fTestLow(s1Cnt, s2Cnt) = fNow;
  end
end

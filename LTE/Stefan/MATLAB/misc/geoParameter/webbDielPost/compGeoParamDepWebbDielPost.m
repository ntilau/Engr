...
%   systemString = ['cd E:\Ortwin\WebbDielectricPostQuarter\ & '...
%     'MeshDistorter WebbDielectricPostQuarter 215e6 6 ' ...
%     num2str(sIntPnts{1}(s1Cnt) + 0.18) ' -dx \w'];
% 
%   system(systemString);
% 
%   % name of new created directory
%   dirName = strcat(modelNameOriginalBase, ...
%     num2str(sIntPnts{1}(s1Cnt) + 0.18), '\');
...
% sOrig{1} = 0.228;
% sOrig{1} = 0.1:0.002:0.26;
% sOrig{1} = -0.08:0.002:0.08;
% 
% % modelNameOriginalBase = ...
...
%   fNameRHS = strcat(modelNameOriginalBase, num2str(sOrig{1}(1) + 0.18), ...
%     '\rhs', num2str(dimCnt-1));
%   rhs(:,dimCnt) = vectorReader(fNameRHS);
%   fNameLvec = strcat(modelNameOriginalBase, num2str(sOrig{1}(1) + 0.18), ...
...
%     num2str(sOrig{1}(s1Cnt) + 0.18), '\system matrix');
...
% grid;
% hold;
% fNameHFSS_Results = 'WebbDielectricPostGeoPar';
% run(fNameHFSS_Results);
% s11hfss = zeros(length(s1), 1);
% for s1Cnt = 1:length(s1)
%   s11hfss(s1Cnt) = S_HFSS{s1Cnt}(1, 1);
% end
% inRange = find((s1 >= 0.1) & (s1 <= 0.26));
% plot(s1(inRange), abs(s11hfss(inRange)), 'rd');
% 
% % error plot
% figHandle = figure;
...
%   posS_Test1(pntCnt) = find(round(sTest{1} * 1e5) ...
%     == round(s1(inRange(pntCnt)) * 1e5));
% end
...
% save webbDielPostdataPic1 sTest s11 s11hfss inRange 

%%
%% frequency sweep
load webbDielPostTestEquiZero

numLeftVecs = 2;
deltaR = 0.03;
% yDist = 0;
% zDist = 0;
freqParam.fExp = 215e6;
freqParam.fMin = 160e6;
freqParam.fMax = 240e6;
freqParam.numPnts = 33;
c0 = 299792.458e3;
k0 = 2 * pi * freqParam.fExp / c0;
freqParam.fCutOff = [1.4989622745620999e+008 1.4989622745620999e+008];
...
fNameMatrix = strcat(modelNameOriginalBase, num2str(deltaR + 0.18), ...
  '\system matrix');
sMatFreq = MatrixMarketReader(fNameMatrix);
fNameMatrixK2 = strcat(modelNameOriginalBase, num2str(deltaR + 0.18), ...
  '\k^2 matrix');

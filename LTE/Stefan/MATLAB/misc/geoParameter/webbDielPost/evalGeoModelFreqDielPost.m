...
geoModelName = ...
  'C:\work\examples\WebbDielectricPostQuarter\WebbDielectricPostQuarter_2.15e+008_6_R_0.18_o7\';
modelName = geoModelName;

% set geometry parameter
deltaR = -0.08:0.001:0.08;
deltaR = 0.07;
% deltaR = -0.04;
% yDist = 0;
geoParams(1).name = 'Radius';
geoParams(1).steps = deltaR;
geoParams(1).numPnts = length(deltaR);
geoParams(1).min = deltaR(1);
geoParams(1).max = deltaR(end);
...
%     s11(kStepCnt, geo1Cnt) = sMat{(kStepCnt - 1) * ...
%       length(geoParams(1).steps) + geo1Cnt}(1, 1);
%   end
% end
% 
% for kStepCnt = 1:freqParam.numPnts
...
%   f(kStepCnt) = freqParam.fMin + (kStepCnt - 1) * deltaF;
% end
% surf(geoParams(1).steps, f, abs(s11));

% save webbDielPost3d geoParams f s11

save webbDielPostFreq_23_o7 geoParams f s11

%%

fNameTest = 'C:\work\examples\patch_antenna\patch_antenna_modRed\patch_antenna_6e+009_EPSILON_RELATIVE_Box1_(4,0)_EPSILON_RELATIVE_Box2_(2,0)_6_intp\sysMatRed_0_0_0';
testMat = readMatFull(fNameTest);
isSym = length(find(testMat - testMat.'));
display(isSym);
isHerm = length(find(testMat - testMat'));
display(isHerm);

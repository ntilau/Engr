function buildRedModelInterpNewPnts(modelName, order, linFreqParamFlag, transposeFlag, orthoFlag)
        ...
        for thisOrder = 1:order
            ...
            for orderCnt = 0:thisOrder
                for pntCnt = 1:size(interpolPntsNow{orderCnt+1},1);
                    ...
                    [Qref] = wcaweImprvd(fact, oneParamModel, ...
                        sol{unredModel.useKrylovSpaces(k)+1}, thisOrder, [], [], orthoFlag);
                    Q = Qref;
                    % modified Gram Schmidt
                    for kColCnt = 1:currentCol
...
clear fact;
rom = projectUnredModel(unredModel, K, transposeFlag);
clear unredModel;
clear K;

function buildRedModelInterpTranspArbExpPnt(modelName, order, linFreqParamFlag, expPnt)
...
[f0, paramNames, paramValInExp, numLeftVecs, abcFlag] = readModParaTxt(fNameModRedTxt);
...
% numInterpolPnts = nchoosek(order + numParams - 1, order);
numVectorsTotal = 0;
...
for k = 1:length(useKrylovSpaces)
    ...
    if numParams == 1
        ...
        [Q U] = wcaweImprvd(fact, oneParamModel, sol{useKrylovSpaces(k)+1}, order, [], []);
        if isempty(K)
            K = Q;
            currentCol = size(Q, 2);
        else
        ...
        for orderCnt = 1:length(interpolPnts)
            for pntCnt = 1:size(interpolPnts{orderCnt}, 1);
                ...
                %         order);
                [Q H] = wcawe(fact, oneParamModel, sol{useKrylovSpaces(k)+1}, order);
                if ~nnz(K)
                    K(:, 1 : size(Q, 2)) = Q;
                    currentCol = size(Q, 2);
                else
...
%%%%%%%%%%%%%
% generate old unreduced order modle
sysMat{1} = sys0 - k0 ^ 2 * k2_mat;
sysMat{2} = -k0 ^ 2 * k2_mat;
%%%%%%%%%%%%%%
...
for k = 1:length(identRed)
    fName = strcat(modelName, 'identRed', num2str(k-1), '.fmat');
    writeMatFull(identRed{k}, fName);
end

for k = 1:numLeftVecs
    writeVector(lVecRed{k}, strcat(modelName, 'leftVecsRed', num2str(k-1), '.fvec'));
    writeVector(redRhs{k}, strcat(modelName, 'redRhs', num2str(k-1), '.fvec'));
end
toc

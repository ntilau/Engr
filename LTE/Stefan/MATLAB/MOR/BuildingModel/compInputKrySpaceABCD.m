...
% sol = cell(size(Model.B{1},2),1);
% for k = 1:size(Model.B{1},2)
%   sol{k} = Fact.Q*(Fact.U\(Fact.L\(Fact.P*Model.B{1}(:,k))));
% end


% iOneParamModel = 0;
% save('UnredModelNew.mat', 'Model');
...
%         fName = ['oneParamModelNew_' num2str(iOneParamModel) '.mat'];

function X = compGenKrySpaceNparamPoly(sysMat, Fact, startVec, order, ...
    numParam, pardisoFlag)
...
for p = 2:length(sysMat)
    if ~isempty(sysMat{p})
        AX = applyMultOp(sysMat{p}, Fact, X(:,1), pardisoFlag);
        proj = X(:,1)'*AX;
...
for n=2:r
    ...
    for matCnt = 2:length(sysMat)
        if ~isempty(sysMat{matCnt})
            sysMatX = applyMultOp(sysMat{matCnt}, Fact, X(:,n), pardisoFlag);

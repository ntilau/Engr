function fNameSpara = modelEvaluationOld(modelName, impedanceFlag, newFileEndingFlag, saveMatlabFlag)
...
redRhsScaled = zeros(length(rhs{1}), numLeftVecs);
if isempty(materialParams)
...
for kStepCnt = 1 : freqParam.numPnts
    ...
    if cSpace
        for pntCnt = 1 : cSpace
            ...
            [L U] = lu(currentMat);
            sol = U \ (L \ redRhsScaled);
            sMat{(kStepCnt - 1) * cSpace + pntCnt} = leftVecs * sol;
        end
    else  % no material dependence, only pure frequency dependence
        [L U] = lu(fMat);
        sol = U \ (L \ redRhsScaled);
        sMat{kStepCnt} = leftVecs * sol;
    end
end
...
if saveMatlabFlag
    save(fNameSpara, 'sMat', 'freqParam', 'materialParams', 'numLeftVecs', 'paramSpace', 'paramNames');
else

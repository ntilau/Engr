function generateSinglePointModel(modelName, order, linFreqParamFlag, fNewExpansion)

% Modification of "buildRedModel.m" (Ortwin). Generates singel-point ROM at
% fNewExpansion parameter.

disp(' ')
disp('Generate single point reduced model...');
...
%% Do adaptions for changing expansion point
% No abc implemented yet !!!

k0 = 2*pi*f0/c0;
kNewExpansion = 2*pi*fNewExpansion/c0;
...
% [xDir,yDir]=find(k2_mat==1);
% k2_mat(xDir,yDir)=0;

sys0 = sys0 + (k0^2- kNewExpansion^2)*k2_mat;
...
%   sysMat{2} = -kNewExpansion^2 * k2_mat;
...
for k = 1:numLeftVecs
  rhs{k} = vectorReader(strcat(modelName, 'rhs', num2str(k-1))) * fNewExpansion/fOrigExpansion ; 
end

function [CS, CT] = BuildFemDDM_epsilon_saarMultipleEpsilon(MESH,xy,nlab,iNODEc,PO,Np,LPEC,LPMC,ndie,material,plane,nuRel,epsRel,ND, sweep)
...
for d = ND
    ...
    for epsCnt = 1 : length(sweep.epsilon)
      MSmtr{epsCnt} = spalloc(NNODE, NNODE, NNODE);   %Subdomain nodes interaction FEM-matrix
      MTmtr{epsCnt} = spalloc(NNODE, NNODE, NNODE);   %Subdomain nodes interaction FEM-matrix
    end
                ...
                if not((iauxi==LPEC & plane == 'H') | (iauxi==LPMC & plane == 'E')) %% J: NOT PEC
                    if not((iauxw==LPEC & plane == 'H') | (iauxw==LPMC & plane == 'E')) %% J: NOT PEC -- K: NOT PEC
                        ...
                        if (iauxj<=NNODEi) & (iauxk<=NNODEi)
                            if mtrFlag
                                MSmtr{imat-1}(iauxj-ibase,iauxk-ibase)  = MSmtr{imat-1}(iauxj-ibase,iauxk-ibase) + cauxSmtr;
                                MTmtr{imat-1}(iauxj-ibase,iauxk-ibase)  = MTmtr{imat-1}(iauxj-ibase,iauxk-ibase) + cauxTmtr;
                            else

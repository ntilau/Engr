function results = solveModel(model, sweep, nmode, xy, PO, Np)
...
for k = 1 : nfreq
    ...
    for ik = 1:sweep.epsilon.N
        sysMatEps = sysMatFreq - k0^2 * sweep.epsilon.epsRel(ik) * model.Tdiel;
...
%             sysMat = sysMatFreq + 1 / sweep.mu.muRel(muCnt) * model.Sdiel - k0^2 * sweep.epsilon.epsRel(ik) * model.Tdiel;
            sysMat = sysMatEps + 1 / sweep.mu.muRel(muCnt) * model.Sdiel;

function [BC,CD]=BuildBCCDDDM(xy,Bi,RZ,PO,a,k0,Zeta0,Np,nmode,plane,nord,iNODEc)
%--------------------------------------------------------------------------
%   BUILDING FUNCTION OF THE INTERACTION MATRICES BLOCKS BETWEEN PORT-MODES
%   AND INTERNAL FIELDS
%--------------------------------------------------------------------------

NNODE = size(xy,2)-iNODEc;
npnm = Np*nmode;
%initializations
    ...
    for ip=1:Np %port index
        ...
        for ie = 1:nel
            ...
            for j=1:nord+1
                ...
                BC(im+1,PO(i1+j-1,ip)-iNODEc) = BC(im+1,PO(i1+j-1,ip)-iNODEc) + integ;
                CD(PO(i1+j-1,ip)-iNODEc,im+1) = CD(PO(i1+j-1,ip)-iNODEc,im+1) - cj * Bi(1,ip) * RZ(1,ip) * integ;
                %LSE1m modes
                for m=1:nmode-1
                    aux = cj * m * piabp;
                    integ = dx * real( exp(aux*x1) * Pjnexpa(nord+1,j,aux*dx) );
                    BC(im+m+1,PO(i1+j-1,ip)-iNODEc) = BC(im+m+1,PO(i1+j-1,ip)-iNODEc) + integ;
                    CD(PO(i1+j-1,ip)-iNODEc,im+m+1) = CD(PO(i1+j-1,ip)-iNODEc,im+m+1) - cj * Bi(m+1,ip) * RZ(m+1,ip) * integ;
                end
            end
        end
    end
        ...
        for ie = 1:nel
            ...
            for j=1:nord+1;
                for m=1:nmode
                    ...
                    BC(im+m,PO(i1+j-1,ip)-iNODEc) = BC(im+m,PO(i1+j-1,ip)-iNODEc) + integ;
                    CD(PO(i1+j-1,ip)-iNODEc,im+m) = CD(PO(i1+j-1,ip)-iNODEc,im+m) + aux * Bi(m,ip) * RZ(m,ip) * integ;
                end
            end
        end
    end


addpath(genpath('C:\work\Matlab'));
set(0, 'DefaultFigureWindowStyle', 'docked');

% mex -LC:\work\LibTest\x64\release -lLibTest -v mexTest.c

%% call compiler for dss interface
%mex -v pardisoinit.c
clear all;
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g dss_sym_c.c
dss_sym_c;

% mex -v yprime.c

%% call compiler for dss interface
%mex -v pardisoinit.c
clear all;
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g dssFactorSymPosDef.c
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g dssSolveSymPosDef.c
% [opt handle] = dssFactorSymPosDef;
% [handle opt ptValue] = dssFactorSymPosDef;
[handle] = dssFactorSymPosDef;
dssSolveSymPosDef(handle);


%% call compiler for pardiso files
clear all;
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g pardiso_sym_c.c
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g pardiso_sym_complex_c.c
pardiso_sym_c;
pardiso_sym_complex_c;

mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g -largeArrayDims pardisoReorderLTE.c
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g -largeArrayDims pardisoFactorLTE.c
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -g -largeArrayDims pardisoSolveLTE.c

%%%%%%%%%%
% Test 1 %
%%%%%%%%%%

dim = 8;
A = zeros(dim);
A = [7 0 1 0 0 2 7 0;
  0 -4 8 0 2 0 0 0;
  1 8 1 0 0 0 0 5;
  0 0 0 7 0 0 9 0;
  0 2 0 0 5 1 5 0;
  2 0 0 0 1 -1 0 5;
  7 0 0 9 5 0 11 0;
  0 0 5 0 0 5 0 5];
A = sparse(A);
mtype = -2;
[iparm pt] = pardisoReorderLTE(mtype, A);
tic
   pardisoFactorLTE(pt, mtype, A, iparm);
toc
neqns = size(A);
x = (1 : neqns(1))';
b =  A*x;
x = zeros(neqns(1),1);
releaseMemory = true;
tic
   pardisoSolveLTE(pt, mtype, A, iparm, b, x, releaseMemory);
toc
display(norm(A * x - b) / norm(b));

%%%%%%%%%%
% Test 2 %
%%%%%%%%%%

dim = 300;
A2 = zeros(dim);
A2 = randn(dim);
A2 = A2 + A2';
A2 = sparse(A2);
mtype = -2;
[iparm2 pt2] = pardisoReorderLTE(mtype, A2);
tic
   pardisoFactorLTE(pt2, mtype, A2, iparm2);
toc
neqns = size(A2);
x2 = (1 : neqns(1))';
b2 = A2*x2;
x2 = zeros(neqns(1),1);
%  solve
releaseMemory = true;
tic
   pardisoSolveLTE(pt2, mtype, A2, iparm2, b2, x2, releaseMemory);
toc
display(norm(A2 * x2 - b2) / norm(b2));

%%%%%%%%%%
% Test 3 %
%%%%%%%%%%
dim = 100;
A3 = rand(dim);
A3 = A3 * A3';
A3 = sparse(A3);
mtype3 = 2;
[iparm3 pt3] = pardisoReorderLTE(mtype3, A3);
tic
   pardisoFactorLTE(pt3, mtype3, A3, iparm3);
toc
neqns = size(A3);
x3 = (1 : neqns(1))';
b3 =  A3 * x3;
x3 = zeros(neqns(1),1);
releaseMemory = true;
tic
   pardisoSolveLTE(pt3, mtype3, A3, iparm3, b3, x3, releaseMemory);
toc
display(norm(A3 * x3 - b3) / norm(b3));


fNameMat = 'C:\work\examples\patch_antenna\patch_antenna_modRed\patch_antenna_6e+009_EPSILON_RELATIVE_Box1_(4,0)_EPSILON_RELATIVE_Box2_(2,0)_11\system matrix_4';
A3 = MatrixMarketReader(fNameMat);
fNameRhs = 'C:\work\examples\patch_antenna\patch_antenna_modRed\patch_antenna_6e+009_EPSILON_RELATIVE_Box1_(4,0)_EPSILON_RELATIVE_Box2_(2,0)_11\rhs0_4';
b3 = vectorReader(fNameRhs);

mtype = 6;
[iparm3 pt3] = pardisoReorderLTE(mtype, A3);
tic
   pardisoFactorLTE(pt3, mtype, A3, iparm3);
toc
neqns = size(A3);
% x3 = (1 + 2j) * (1 : neqns(1))';
% b3 = A3*x3;
% x3 = zeros(neqns(1),1);
x3 = (1 + j) * ones(neqns(1),1);
%  solve
releaseMemory = true;
tic
   pardisoSolveLTE(pt3, mtype, A3, iparm3, b3, x3, releaseMemory);
toc
display(norm(A3 * x3 - b3) / norm(b3));


%% Cpp Files
clear all;
close all;

mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -lmkl_dll -lmkl_ipf -v -g pardiso_sym_complex_cpp.cpp

mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -largeArrayDims pardisoReorderLTE_Cpp.cpp
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -largeArrayDims pardisoFactorLTE_Cpp.cpp
mex -I"C:\Program Files\Intel\MKL\9.1.027\include" -L"C:\Program Files\Intel\MKL\9.1.027\em64t\lib" -lmkl_solver -lmkl_em64t -lmkl_lapack -llibguide40 -v -largeArrayDims pardisoSolveLTE_Cpp.cpp


%%%%%%%%%%
% Test 1 %
%%%%%%%%%%

dim = 8;
A = zeros(dim);
A = [7 0 1 0 0 2 7 0;
  0 -4 8 0 2 0 0 0;
  1 8 1 0 0 0 0 5;
  0 0 0 7 0 0 9 0;
  0 2 0 0 5 1 5 0;
  2 0 0 0 1 -1 0 5;
  7 0 0 9 5 0 11 0;
  0 0 5 0 0 5 0 5];
A = sparse(A);
mtype = -2;
[iparm pt] = pardisoReorderLTE_Cpp(mtype, A);
tic
   pardisoFactorLTE_Cpp(pt, mtype, A, iparm);
toc
neqns = size(A);
x = ones(neqns(1),1);
x = (1 : neqns(1))';
b =  A*x;
x = zeros(neqns(1),1);
releaseMemory = true;
tic
   pardisoSolveLTE_Cpp(pt, mtype, A, iparm, b, x, releaseMemory);
toc
norm(A*x-b)/norm(b)


%%%%%%%%%%
% Test 2 %
%%%%%%%%%%

dim = 300;
A2 = zeros(dim);
A2 = randn(dim);
A2 = A2 + A2';
A2 = sparse(A2);
mtype = -2;
[iparm2 pt2] = pardisoReorderLTE_Cpp(mtype, A2);
tic
   pardisoFactorLTE_Cpp(pt2, mtype, A2, iparm2);
toc
neqns = size(A2);
x2 = ones(neqns(1),1);
x2 = (1 : neqns(1))';
b2 = A2*x2;
x2 = zeros(neqns(1),1);
%  solve
releaseMemory = true;
tic
   pardisoSolveLTE_Cpp(pt2, mtype, A2, iparm2, b2, x2, releaseMemory);
toc
norm(A2*x2-b2)/norm(b2)


%%%%%%%%%%
% Test 3 %
%%%%%%%%%%

% dim = 500;
% A3 = zeros(dim);
% A3 = randn(dim) + j * randn(dim);
% A3 = A3 + A3.';
% A3 = sparse(A3);

fNameMat = 'C:\work\examples\patch_antenna\patch_antenna_modRed\patch_antenna_6e+009_EPSILON_RELATIVE_Box1_(4,0)_EPSILON_RELATIVE_Box2_(2,0)_11\system matrix_4';
A3 = MatrixMarketReader(fNameMat);
fNameRhs = 'C:\work\examples\patch_antenna\patch_antenna_modRed\patch_antenna_6e+009_EPSILON_RELATIVE_Box1_(4,0)_EPSILON_RELATIVE_Box2_(2,0)_11\rhs0_4';
b3 = vectorReader(fNameRhs);
% save linearSystem;

mtype = 6;
[iparm3 pt3] = pardisoReorderLTE_Cpp(mtype, A3);
tic
   pardisoFactorLTE_Cpp(pt3, mtype, A3, iparm3);
toc
neqns = size(A3);
% x3 = (1 + 2j) * (1 : neqns(1))';
% b3 = A3*x3;
% x3 = zeros(neqns(1),1);
x3 = (1 + j) * ones(neqns(1),1);
%  solve
releaseMemory = true;
tic
   pardisoSolveLTE_Cpp(pt3, mtype, A3, iparm3, b3, x3, releaseMemory);
toc
display(norm(A3 * x3 - b3) / norm(b3));

...
/* -------------------------------------------------------------------- */
  ...
  
  {
    ...
    //     A_ia_sym = (int *) mxCalloc(n_row + 1, sizeof(int));
    
    //     for (i = 0; i <= n_col; i++)
    //       printf("i = %d, A_ia[i] = %d\n", i, A_ia[i]);
    //     for (i = 0; i < nnz; i++)
    //       printf("i = %d, A_valuesR[i] = %e\n", i, A_valuesR[i]);
    //     for (i = 0; i < nnz; i++)
    //       printf("i = %d, A_ja[i] = %d\n", i, A_ja[i]);
    //     printf("nnz_sym = %d\n", nnz_sym);
    //     printf("n_row = %d\n", n_row);
...
//     A_ia_sym = (int *) calloc(n_row + 1, sizeof(int));
    A_ja_sym = new int[nnz_sym];
    A_ia_sym = new int[n_row + 1];
    
    if (mtype == -2) {
//       A_values_sym = (double *) calloc(nnz_sym, sizeof(double));
      A_values_sym = new double[nnz_sym];
...
//       A_values_sym_c = (MKL_Complex16 *)calloc(nnz_sym, sizeof(MKL_Complex16));
      A_values_sym_c = new MKL_Complex16[nnz_sym];

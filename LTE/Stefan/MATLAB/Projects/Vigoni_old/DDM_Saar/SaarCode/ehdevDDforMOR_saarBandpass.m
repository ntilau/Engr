function [unredModel, solVecs, results, sweep, nmode, xy, PO, Np, MESH] = ehdevDDforMOR_saarBandpass(mesh, sweep, flagout)
...
%     epsilon{epsCnt} = [sweep.epsilon{epsCnt}.start : (sweep.epsilon{epsCnt}.stop - ...
...
[BLOCK, CS, CT] = BuildFemDDM_saarBandpass(MESH,xy,nlab,iNODEc,PO,Np,LPEC,LPMC,ndie,material,plane, nuRel, epsRel, ND);
...
% build global S and T matrices
[S, T] = getGlobalST_Bandpass(CS, CT) ;
...
[SaarBlock, CS_saar, CT_saar] = getSubDomMatsBandpass(compMatS, compMatT, ND);
...
for k=1:nfreq
  ...
  % scale rhs
  rhsFreq = rhs * freqScalMat;
  sysMatFreq = unredModel.S - k0^2 * unredModel.T;
  % calculate solution
  sol = sysMatFreq \ rhsFreq;
  % get impedance matrix
  zMat = unredModel.leftVec' * sol;
  % calculate s matrices and store them
  results.sMat{k} = inv(zMat - eye(size(zMat, 1))) * (zMat + eye(size(zMat, 1)));
...
%         
%         sol_saar = SolverDDM_saarLastDomPlusShur(SaarBlock, SC_saar, CS_saar, CT_saar, k0, nuRel, ...

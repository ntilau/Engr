function results = solveModelMultipleEpsilon(model, sweep, nmode, xy, PO, Np, muRel)
...
for k = 1 : nfreq
  ...
  for epsCnt1 = 1:sweep.epsilon{1}.N
    ...
    for epsCnt2 = 1:sweep.epsilon{2}.N
      ...
      sysMat = sysMatEps1 + - k0^2 * sweep.epsilon{2}.epsRel(epsCnt2) * model.Tdiel{2};
      ...
      results.sMat{k, epsCnt1, epsCnt2} = inv(zMat - eye(size(zMat, 1))) * (zMat + eye(size(zMat, 1)));
    end
  end
end

function [SaarBlock, SCdiel_saar] = ShurC_epsilon_saarMultipleEpsilon(SaarBlock, SC_saar, CS_saar, CT_saar, k0, nuRel, epsRelArr, ND)
...
for id = ND
    ...
    for epsCnt = 1 : length(epsRelArr)
      M = M + nuRel * SaarBlock{id}.MSmtr{epsCnt} - k0^2 * epsRelArr(epsCnt) * SaarBlock{id}.MTmtr{epsCnt};
    end

...
interpolationPnts.frequency.start = 10;
...
interpolationPnts.epsilon.stop(1)   = 5;%6;
interpolationPnts.epsilon.N(1)      = 5;%11;
...
interpolationPnts.mu.stop(1)        = 5;
interpolationPnts.mu.N(1)           = 5;
interpolationPnts.mu.mlabel(1)      = 2;
interpolationPnts.mu.domain         = -1;

flagout = 1;

[unredModel, solVecs, results, sweep, nmode, xy, PO, Np, MESH] = ehdevDDforMOR_saar(mesh, interpolationPnts, flagout);
...
% build ROM
t1 = cputime;
ROM = buildROM(unredModel, solVecs);
t2 = cputime;
...
%% solve ROM
sweep.frequency.start = 10e9;
sweep.frequency.stop  = 12e9;
sweep.frequency.N     = 10;

sweep.epsilon.materials = 1;
sweep.epsilon.start(1)  = 1;%4
sweep.epsilon.stop(1)   = 5;%6;
sweep.epsilon.N(1)      = 51;%11;
...
tic;
results = solveModel(ROM, sweep, nmode, xy, PO, Np);
toc;
...
%% solve unredModel
t1 = cputime;
results = solveModel(unredModel, sweep, nmode, xy, PO, Np);
t2 = cputime;
totalTime = t2 - t1;
display(totalTime);

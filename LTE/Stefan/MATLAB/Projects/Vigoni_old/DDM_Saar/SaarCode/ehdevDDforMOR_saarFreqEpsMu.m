function [unredModel, solVecs, results, sweep, nmode, xy, PO, Np, MESH] = ehdevDDforMOR_saarFreqEpsMu(mesh, sweep, ...
  flagout)
...
mu = cell(length(sweep.mu), 1);
for muCnt = 1 : length(sweep.mu)
  mu{muCnt} = sweep.mu{muCnt}.start;
  if sweep.mu{muCnt}.N > 1
    mu{muCnt} = linspace(sweep.mu{muCnt}.start, sweep.mu{muCnt}.stop, sweep.mu{muCnt}.N);
  end
end
...
% sweep.nu.nuRel = 1./mu;
for muCnt = 1 : length(sweep.mu)
  sweep.mu{muCnt}.nuRel = 1 ./ mu{muCnt};
end
...
[CSd, CTd] = BuildFemDDM_epsilon_saarMultipleEpsilon(MESH,xy,nlab,iNODEc,PO,Np,LPEC,LPMC,ndie,material,plane,nuRel,epsRel,ND, sweep);

% build portmodes
portModes = getPortModes(xy, PO, Np, nmode, plane, nord);

% build globla S and T matrices
[S, T, Sdiel, Tdiel] = getGlobalST_MultipleEpsilon(CS, CT, CSd, CTd) ;
...
%     tic;
    for epsCnt = 1:sweep.epsilon{1}.N

      for nuCnt = 1:sweep.mu{1}.N

        epsRelArr = [sweep.epsilon{1}.epsRel(epsCnt)];
        nuRelArr = [sweep.mu{1}.nuRel(nuCnt)];
        
        sol_saar = SolverDDM_saarLastDomPlusShurFreqEpsMu(SaarBlock, SC_saar, CS_saar, CT_saar, k0, nuRelArr, ...
          epsRelArr, ND, rhsFreqExtend);

        % store solution for MOR
        solVecs{k, epsCnt, nuCnt} = sol_saar;

        % get impedance matrix
        zMat = sol_saar(dimCompMat - (Np * nmode - 1) : end, :);

        % calculate s matrices and store them
        results.sMat{epsCnt, nuCnt, k} = inv(zMat - eye(size(zMat, 1))) * (zMat + eye(size(zMat, 1)));
        % end

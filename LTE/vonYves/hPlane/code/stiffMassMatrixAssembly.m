...
function [stiffMatrix, massMatrix] = stiffMassMatrixAssembly(project, scalarOrder)
%% read element matrices
ElementMatrix = elementMatrixReader(project, scalarOrder);
% scalar mass matrix
...
discreteGrad = getGradientOperator(scalarOrder);
...
for faceCnt = 1:project.faceDim
    ...
    [epsilon_r mu_rInv] = faceMaterialProperties(project, faceCnt);
    ...
    % x-coefficient   
    T_ss(1:scalarDim,1:scalarDim) = epsilon_r / mu_rInv * detJ * MasterT_ss; 
    ...
    for i=1:3
        ...
        edgeIndex(i).scalar = project.geo.index(globalEdgeId).scalarDomain;        
    end

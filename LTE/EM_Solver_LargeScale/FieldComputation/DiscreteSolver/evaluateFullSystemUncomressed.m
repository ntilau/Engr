...
function [sMat, zMat, dof, sol, solveTime] = ...
    evaluateFullSystemUncomressed(Model)
...
% scale vectors related to uncompressed ports
for k = 1:Model.nPorts
    if nnz(lhs(:,k)) > 1
        fprintf('\n\t- Port %d is uncompressed.', k);
        lhs(:,k) = rhs(:,k) / (1j * Model.kExp * Model.eta0);
        rhs(:,k) = 1j * rhs(:,k);
    end
end  

function h = plotImpErrorNorm(Rom, Error, fontSize)
...
for iRom = 1:nRoms    
    ErrorNorm = computeErrorNorms(Error(iRom).zMat);
    ...
    if isfield(Rom{iRom}, 'ErrorEst') && ~isempty(Rom{iRom}.ErrorEst) &&...
        ...
        ErrorEstNorm = computeErrorNorms(Rom{iRom}.ErrorEst.impedance);
...
figName = sprintf('Norm of Estimated Error in Z-Parameters');
...
ylabel('$E(\mathbf{\mathsf Z}; \mathcal B_f)$ (Z Formulation)', ...
